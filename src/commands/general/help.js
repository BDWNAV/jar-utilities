...
module.exports.run = async (client, message, args) => {
  ...
  setTimeout(() => {
    // There is a 1ms timeout here because node goes through the rest of the code before reading the directory for some reason, this just "prevents" that from happening
    if (!args.length) {
      // If there aren't any categories provided, show a menu with all the categories
      ...
      categories.forEach((category) => {
        // for each category in the categories variable, add an embed field for that category
        help.addField(
      ...
      commands.forEach((command) => {
        ...
        // If the maximum number of arguments is -1 (unlimited), show the user "Unlimited"
        else maxArgs = cmd.help.maxArgs; // If it isn't, show them the number of arguments
        if (typeof cmd.help.aliases === "string")
          cmd.help.aliases = [cmd.help.aliases];
        else;

        help.addField(
          cmd.help.name,
          `*${cmd.help.description}*\n\n**Usage:** ${prefix}${cmd.help.name} ${
            cmd.help.usage
          }\n**Aliases:** ${
            cmd.help.aliases ? cmd.help.aliases.join(", ") : "None"
          }\n**Owner Only:** ${cmd.help.ownerOnly}\n**Minimum Arguments:** ${
            cmd.help.minArgs
          }\n**Maximum Arguments:** ${maxArgs}`,
          true
        );
      });
      channel.send({ embeds: [help] });
